(function () {
  ...
  const storage = name => (db[name] || set$1(name));

  const STORAGE_KEY = ":todo-heresy";
  const data = name => {
  ...
  const update$1 = (name, data) => {
    db[name] = data;
  };

  function handleEvent$1() {
    const {name} = this;
    localStorage.setItem(name, JSON.stringify(db[name]));
  }

  function set$1(name) {
    addEventListener('beforeunload', {name, handleEvent: handleEvent$1}, false);
  ...
  const renderer = (state) => {
    ...
    render$1(appEl, html$1`<Todo .data=${state} .intents=${intents} />`);
  };
